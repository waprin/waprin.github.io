...
I first learned backend web with Perl / CGI, then moved on to PHP. PHP was attractive because you just plop the page down and have a website hooked up to a database, but early PHP also created massive spaghetti code and had many security issues. Python was still a hipster language in the early 2000s but when I first discovered Django it blew my mind how much more productive you could be with its “batteries included” approach. I especially admired how easy it was to define data models and automatically get useful admin interface to manage them.

While my first few jobs out of college were Java and C++, I pushed to work on Python and Django and when I joined Google I led all the documentation around using Django on Google Cloud Platform, including docs on how to run it on App Engine (managed and flexible), Compute Engine, and Kubernetes / Container engine. My only Github repo to attract non-negligible attention is a [Django on Kubernetes sample](https://github.com/waprin/kubernetes_django_postgres_redis). And in past few years the vast majority of my development focus has been on Python.
...
- Because I wanted infinite scroll to make the website more modern, I had to render the grid components as they scrolled. It would have been better to do this server-side since it would be faster, but it also needed to be done client-side after the timezone was updated.
